...
// so we lift the layout ourselves: set --kb-inset to the keyboard height and
// the fixed app shell's bottom rides up above it (see .app-shell in CSS). The
// onChange callback lets the caller refit xterm once the layout settles.
export function useKeyboardInset(onChange?: () => void) {
  useEffect(() => {
    ...
    void Keyboard.addListener('keyboardWillShow', (info) => {
      root.style.setProperty('--kb-inset', `${info.keyboardHeight}px`);
      onChange?.();
    }).then((h) => (showHandle = h));
